...
func getResult(lines utils.LineParser) (int, error) {
	...
	sizeDisk := 70000000
	sizeNeeded := 30000000
	sizeUsed := root_node.Size()
	sizeToFree := -(sizeDisk - sizeUsed - sizeNeeded)

	return FindSmallestDirSizeToFree(root_node, sizeToFree), nil
}

func FindSmallestDirSizeToFree(node Node, leastSize int) int {
	if !node.IsDir() {
		panic("Needs to be called with dir")
	}
	result := node.Size()
	return findSmallestDirSizeToFree(node, leastSize, result)
}

func findSmallestDirSizeToFree(node Node, leastSize int, result int) int {
	if (node).IsDir() && node.Size() > leastSize && node.Size() < result {
		result = node.Size()
	}
	for _, child := range node.Children() {
		result = findSmallestDirSizeToFree(*child, leastSize, result)
	}
	return result
}
