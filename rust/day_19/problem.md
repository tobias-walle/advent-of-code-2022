...
[geodes][1] around the pond. Perhaps you could use the obsidian to create some
...
9 geodes, so its quality level is `*9*`. The second blueprint has ID 2 and can
open 12 geodes, so its quality level is `*24*`. Finally, if you *add up the
...
In 32 minutes, the largest number of geodes blueprint 1 (from the example above)
can open is `*56*`. One way to achieve that is:
...
[1]: https://en.wikipedia.org/wiki/Geode
