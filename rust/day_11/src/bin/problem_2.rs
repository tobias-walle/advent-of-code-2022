...
fn solve_problem(input: &str) -> Result<u128> {
    ...
    let mut inspections: HashMap<MonkeyId, u128> = HashMap::new();

    let rounds = 10000;

    let modulo: u128 = monkeys.values().map(|m| m.test.divisible_by).product();
